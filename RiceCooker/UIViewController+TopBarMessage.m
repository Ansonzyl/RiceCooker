...
#define kTopBarHeight 25.0f
#define kDefaultDimissDelay 1.5f
#define kHeight 0
...
- (void)resetViews
{
    if (!__defaultTopMessageConfig) {
        __defaultTopMessageConfig = [@{kDXTopBarBackgroundColor : [UIColor colorWithWhite:0xfff alpha:0.7], kDXTopBarTextColor : UIColorFromRGB(0xff5c5c), kDXTopBarTextFont : [UIFont fontWithName:@"HelveticaNeue-Medium" size:12.0]} mutableCopy];
    }
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    if (newSuperview) {
        self.alpha = 1;
        self.label.textColor = UIColorFromRGB(0xff5c5c);
        CGRect selfFrame = self.frame;
        selfFrame.origin.y -= CGRectGetHeight(selfFrame);
        self.frame = selfFrame;
        selfFrame.origin.y = kHeight;
...
- (void)showTopMessage:(NSString *)message topBarConfig:(NSDictionary *)config dismissDelay:(float)delay withTapBlock:(dispatch_block_t)tapHandler
{
    ...
    if (!topV) {
        topV = [[TopWarningView alloc] initWithFrame:CGRectMake(0, kHeight, CGRectGetWidth(self.view.bounds), kTopBarHeight)];
        objc_setAssociatedObject(self, &TopWarningKey, topV, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    topV.frame = CGRectMake(0, kHeight, CGRectGetWidth(self.view.bounds), kTopBarHeight);
